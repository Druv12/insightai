...
app.post('/api/analyze', verifyToken, async (req, res) => {
  ...
  try {
    ...
    const industryType = detectIndustryType(availableColumns);  // ← Only ONE declaration!
...
res.json({ 
  ...
  schemaMetrics: schemaMetrics,        // ← NEW
  availableColumns: availableColumns,  // ← NEW
  sqlQueries: sqlQueries,              // ← NEW
  formattedSQL: formattedSQL,          // ← NEW
  metadata: {
    totalRevenue: validation.totalRevenue,
    rowCount: validation.rowCount,
