...
FULL DATA: ${JSON.stringify(csvData)}`;
    ...
    // CONCISE FORMAT (ChatGPT's recommendation)
    const concisePrompt = `Analyze this ${industryType} dataset and answer: "${question}"

${dataContext}

${validation.warnings.length > 0 ? 'WARNINGS: ' + validation.warnings.join(', ') : ''}

CRITICAL FORMAT REQUIREMENT - Use this EXACT structure:

📊 KEY INSIGHTS (3 bullets max, with actual numbers):
• [Insight 1 with specific metrics]
• [Insight 2 with trend]
• [Insight 3 with comparison]

🎯 TOP RECOMMENDATION:
[One clear, actionable step]

💰 EXPECTED IMPACT:
[Measurable outcome if recommendation is followed]

RULES:
- Be CONCISE - no long paragraphs
- Include ALL specific numbers (revenue, growth %, quantities) in lakhs/crores
- One short recommendation with measurable impact
- Professional but direct tone
Use emojis: 📊📈💰🎯⚠️🔥`;

    const answer = await callGroqAPI([
      { role: 'system', content: 'You are a concise business analyst. Always use the exact format provided.' },
